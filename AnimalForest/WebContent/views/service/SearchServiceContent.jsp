...
<body>
	...
	<div class="container">
		<div class="row">
			...
			<c:forEach items="${pageData.pageList}" var="pageData" varStatus="i">
				<div class="col-sm-4">
					<div class="panel panel-default" id="listBox">
						...
						<div class="panel-body">
							<img src="/img/${pageData.fileName }"
								style="width: 300px; height: 300px;">
						</div>
						<div class="panel-footer">
							<p>
								글번호 : ${pageData.board_No }<br> 일시 : ${pageData.service_Fr}
								- ${pageData.service_To} <br> 모집인원 : ${pageData.volunteer}명<br>
								신청자격 :
								<c:if test="${pageData.qualfication eq '0'}">제한없음<br>
								</c:if>
								<c:if test="${pageData.qualfication ne '0'}">${pageData.qualfication}살 이상<br>
								</c:if>
							</p>

							<a href="/selectOneContent?board_No=${pageData.board_No }&type=0"><button
									type="button" class="btn btn-success" id="form-btn">신청하기</button>
								<input type="hidden" name="boardNo" value="${pageData.board_No}" />
							</a>
						</div>
					</div>
				</div>
	...
	<script>
		...
		/* $("#form-btn").each(function(index, item) {
			console.log("TEST");
			$("#listBox").addClass("overEffact");
		}, function() {
			$("#listBox").removeClass("overEffact");
